...
class PurchaseOrderDialog(QDialog):
    ...
    def _generate_po_number(self) -> str:
        """Generate PO number in format: PO-{BranchID}{YYMMDD}-####"""
        ...
        # Format: PO-{BranchID}{YYMMDD}-####
        prefix = f'PO-{branch_id}{date_part}'
        
        # Find the highest sequence number for this prefix
        latest_po = (PurchaseOrder
                    .select()
                    .where(PurchaseOrder.po_number.startswith(prefix))
                    .order_by(PurchaseOrder.po_number.desc())
                    .first())
        
        if latest_po:
            # Extract sequence number from PO-{BranchID}{YYMMDD}-####
            try:
