...
class TicketReceiptGenerator:
    ...
    def _generate_thermal_html(self, ticket_data, print_format):
        ...
        if self.settings and self.settings.ticket_terms:
            for term in self.settings.ticket_terms.split('\n'):
                if term.strip():
                    html += f"<div class='terms'>- {term.strip()}</div>"
        else:
            terms = [
    ...
    def _generate_a5_html(self, ticket_data):
        ...
        if self.settings and self.settings.ticket_terms:
            for t in self.settings.ticket_terms.split('\n'):
                if t.strip():
