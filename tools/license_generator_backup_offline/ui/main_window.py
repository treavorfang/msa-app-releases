...
class LicenseGeneratorWindow(QMainWindow):
    ...
    def _get_next_invoice_number(self):
        ...
        last_invoice = self.history.get_last_invoice_number()
        today = datetime.now()
        ...
        # Try to increment
        try:
            return self._increment_invoice_number(last_invoice)
        except:
            return f"INV-{today.strftime('%Y')}-{today.strftime('%m%d')}-001"
    ...
    def _create_license_group(self):
        ...
        # Action Buttons Section (Integrated into License Configuration)
        row += 1
        ...
        self.pdf_btn.setMinimumHeight(30)
        ...
        layout.addLayout(buttons_layout, row, 0, 1, 2)
    ...
    def _create_invoice_group(self):
        ...
        # Row 2: Currency & Amount & Discount
        ...
        self.currency_combo.setFixedWidth(60)
        ...
        self.amount_input.setToolTip("Amount")
        amount_layout.addWidget(self.amount_input)
        
        self.discount_input = QLineEdit()
        self.discount_input.setPlaceholderText("Disc.")
        self.discount_input.setToolTip("Discount")
        self.discount_input.setFixedWidth(60)
        amount_layout.addWidget(self.discount_input)
    ...
    def _create_action_buttons(self):
        """Deprecated: Buttons moved to License Group"""
        return QHBoxLayout()
    ...
    def _generate_license(self):
        ...
        try:
            ...
            if discount:
                license_data['amount'] = f"{currency} {raw_amount} (Disc: {discount})"
                license_data['discount'] = discount
            else:
    ...
    def _increment_invoice_number(self, current):
        """Increment invoice number, resetting sequence if date changes"""
        # Format: INV-YYYY-MMDD-XXX
        if not current:
            return self._get_next_invoice_number()
        ...
        if len(parts) == 4:
            ...
            if parts[1] == today_year and parts[2] == today_mmdd:
                try:
                    num = int(parts[3]) + 1
        ...
        # Reset if date didn't match or error
        return f"INV-{today_year}-{today_mmdd}-001"
